...
smoke.suite('packed smoque tarball installs a runnable bin', { tags: ['package'] }, async (t) => {
    ...
    await t.step('installed smoque bin reports version', async () => {
        ...
        const version = tarball.version;
        if (version === undefined) {
            return t.fail('packed smoque artifact did not report a version');
        }
        expect.value(result.stdout.trim()).toBe(version);
    });
...
async function attachInstallContext(
    t: SmokeContext,
    tarballPath: string,
    consumerPath: string,
    binPath: string,
