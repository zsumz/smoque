...
test('t.ports reserves unique loopback ports and builds env maps', async () => {
    ...
    try {
        smoke.suite('ports', async (t) => {
            ...
            assert.equal(api.url('/health'), `http://127.0.0.1:${String(api.port)}/health`);
            ...
            assert.equal(result.stdout.trim(), `${String(api.port)}:demo:missing`);
        });
...
test('command failures include reserved port diagnostics', async () => {
    ...
    let port: ReservedPort | undefined;
