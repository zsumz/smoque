...
test('t.fixture.fromTemplate copies into provided work dirs and reports missing templates', async () => {
    ...
    try {
        ...
        assert.equal(result.status, 'passed');
        await assert.rejects(async () => access(join(root, '.smoque', 'template-fixture')), /ENOENT/u);
        ...
        assert.match(suite?.error?.message ?? '', /Fixture template directory not found/u);
        assert.equal(suite?.error?.details?.template, join(root, 'missing-template'));
        assert.equal(suite.error.details.destination, join(root, 'copy'));
    } finally {
        await rm(root, { recursive: true, force: true });
    }
});
