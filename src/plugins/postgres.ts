...
import { postgresConnect } from './postgres/connect.js';
import { postgresStart } from './postgres/start.js';
import type {
...
export default function postgresPlugin(): SmokePlugin {
    return definePlugin({
        ...
        register(registry) {
            registry.action(
                'postgres.check',
                async (t, options) => postgresCheck(
                    t,
                    options as PostgresCheckOptions | undefined,
                ),
            );
            registry.resource(
                'postgres.connect',
                async (t, options) => postgresConnect(
                    t,
                    options as PostgresConnectOptions,
                ),
            );
            registry.resource(
                'postgres.start',
                async (t, options) => postgresStart(
                    t,
                    options as PostgresStartOptions | undefined,
                ),
            );
        },
    });
